...
# FLOOR, not what resolves — read Gemfile.lock for that. TWO floors matter,
# and the higher one wins. Local log capping starts at 0.33 (`studio.logger`);
# an app locked below it grows dev/test logs to Rails' 100 MB default,
# silently. But this document also instructs APIs that landed later, and the
# highest is `Studio::GeoDetection` at 0.57.0 — measured across every
# `Studio::` / `Studio.` / `config.` token these pages name that CHANGELOG.md
# dates. Re-derive it whenever you add an instruction here: a lock below the
# floor raises NameError at boot while the pin itself still looks satisfied.
...
The generated workflow installs a short fixed list and nothing else — on a Rails
8.1 app that is `libpq-dev` and `libvips`:
...
that runs tests.** How many jobs that is depends on your Rails version, and this
engine supports both (`studio-engine.gemspec:24` — `rails >= 7.2, < 8.2`):

| Generated by | Test jobs | Install steps |
|---|---|---|
...
### System tests — the lane runs; the tests may not exist

`rails new` wires a `test:system` invocation into CI and puts `capybara` +
`selenium-webdriver` in the Gemfile. **What sits behind that invocation depends on
your Rails version**, and the two versions fail in opposite directions:

| Generated by | `test/system/` + `application_system_test_case.rb` | A fresh app's lane |
|---|---|---|
| railties 7.2 (`app_generator.rb:254-258`, called at `:459`) | **created** — the call site is `build(:system_test) if depends_on_system_test?`, true for a plain `rails new` | **GREEN**, having run zero tests |
| railties 8.1 (`:257-262`) | **not created** — guarded by `devcontainer? && depends_on_system_test?` | **RED**: `cannot load such file -- test/system` |

Note the CI shape differs too: 7.2 runs `test:system` inside its single `test` job
(`ci.yml.tt:131`), while 8.1 emits a separate `system-test` job.

Either way the lane is not testing your app, and **the green one is the harder to
notice** — a red lane announces itself. Check yours before trusting it:

```bash
git ls-files test/system     # `.keep` alone means it has never tested anything
```

On 8.1 you must create both files; on 7.2 you already have them and need only add
a real test:
...
`application_system_test_case.rb` are scaffolded only behind a
`devcontainer? && depends_on_system_test?` guard (`railties app_generator.rb:257-262`, 8.1.3.1 — Rails 7.2 has no such guard),
...
one.** (Two of these you cannot re-fetch: rolio and acquisition-studio have
unreachable remotes, so their rows are only as current as the local clone.) Every app below runs `test:system` in CI (e.g.
...
creates it — and `application_system_test_case.rb` with it — for any plain
`rails new` (`railties 7.2.3.1 app_generator.rb:254-258`, called at `:459` under
`depends_on_system_test?`), so a 7.2-era app inherits a green-forever lane without
choosing one. Do not read those
