...
class StylePageTest < ActiveSupport::TestCase
  ...
  # --- 11. sections group by SUBJECT, not by moment ---------------------------
  #
  # The Onboarding section folded into Profile on 2026-08-24. It grouped cards by
  # WHEN they are shown ("just after signup"), which put the birthday card in one
  # section and the picture cards in another despite both being about who the
  # account is. Grouping by subject also let the wallet step move to the wallet
  ...
  # The specimen cards a section renders, in page order. Read off the open-button
  # aria-label, which is the one place a card's own name appears verbatim — the
  # visible header is uppercased by CSS and the reference prose mentions other
  # cards by name.
  def section_cards(heading)
    section_html(heading).scan(/aria-label="Open the (.+?) modal"/).flatten
  end
  ...
  test "the Onboarding section is gone, folded into Profile" do
    doc = Nokogiri::HTML(render_index)
    headings = doc.css("h3").map { |n| n.text.strip }
    assert_not_includes headings, "Onboarding",
      "Onboarding grouped by MOMENT; its cards moved to the sections that own their subject"
  end

  test "the Profile section carries who-the-account-is, in order" do
    assert_equal PROFILE_ORDER, section_cards("Profile"),
  ...
  # --- RETIRED — the credential-control gate reader -------------------------
  #
  # This section held a string reader for ONE credential control's open tag —
  # open_tag_containing and credential_visibility_gate, their constants
  # CREDENTIAL_CONTROLS, WALLET_CTA_ANCHOR and OPEN_TAG — and the two tests that
  # exercised it. Its only live caller was a test that read style/modals/_auth,
  # retired with that mirror on 2026-09-09 (PR #319), when its weaker sibling
  # x_show_expressions also went. From then on the two tests ran the reader
  # against markup they built themselves: they guarded the helper and nothing
  # this engine ships. The section's own note asked to be deleted once the
  # reader had a home with a live caller, and it has one now.
  #
  # WHERE IT LIVES NOW: turf-monster test/views/auth_submitting_coercion_test.rb,
  # against turf's real sign-in card. The five-invariant walk, the decoy-count
  # refusal and both self-tests went with it. The one change is that the
  # attribute it reads is a parameter, because that card locks each credential
  # with :disabled instead of gating it with x-show.
